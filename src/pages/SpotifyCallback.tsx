 ...
 * accessToken in sessionStorage, then navigates to /music-hub.
 */
export default function SpotifyCallback() {
  ...
  useEffect(() => {
    ...
    // Exchange code → access token via backend (keeps client_secret secure)
    fetch('/api/spotify/token', {
      ...
      .then(data => {
        if (data.accessToken) {
          // Store the actual Spotify access token — used for direct API calls
  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--bg-primary, #0a0a14)' }}>
      <div
        ...
        style={{
          ...
          animation: 'mh-popin 0.3s ease-out',
        }}>
        {/* Spotify Logo */}
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-5"
          style={{ background: '#1DB954', boxShadow: '0 0 30px rgba(29,185,84,0.4)' }}>
          ...
          <div className="flex justify-center mb-4 gap-2">
            {[0, 1, 2, 3].map(i => (
              <span key={i} className="w-2 h-2 rounded-full animate-pulse"
                style={{ background: '#1DB954', animationDelay: `${i * 0.15}s` }} />
            ))}
          </div>
        )}
      ...
      <style>{`@keyframes mh-popin { from { opacity:0; transform:scale(0.9); } to { opacity:1; transform:scale(1); } }`}</style>
    </div>
  );
}
