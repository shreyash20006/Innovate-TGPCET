...
export default function HeroCanvas() {
  ...
  useEffect(() => {
    ...
    const inner = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0xff0066, transparent: true, opacity: 0.04 })
    );
    scene.add(inner);
    ...
    // Theme observer — update material colors without hooks
    const applyTheme = () => {
      const isLight = document.body.classList.contains('theme-light');
      lineMat.color.setHex(isLight ? 0xCC0033 : 0xff0066);
      lineMat.opacity = isLight ? 0.9 : 0.7;
      ringMat.color.setHex(isLight ? 0x005577 : 0x00cfff);
    };
    applyTheme();
